...
const Manifests = function() {
  ...
  var getScan = Promise.promisify(hbase.getScan);
  ...
  function makeRowkey (public_key, sequence) {
    return [
      public_key,
      utils.padNumber(sequence, SEQ_PAD)
    ].join('|');
  }
  ...
  function combineValidatorManifests(public_key) {
    ...
    return getScan.call(hbase, {
      ...
      for (const manifest of resp) {
        if (manifest.rowkey.split('|').length === 3) {
          ...
          // update cache count
          manifests[makeRowkey(manifest.master_public_key, manifest.sequence)] = {
      ...
      for (const seq in seqs) {
        puts.push(hbase.putRow({
          table: 'manifests_by_validator',
          rowkey: makeRowkey(seqs[seq].master_public_key, seqs[seq].sequence),
          columns: seqs[seq]
        }))
      }
  ...
  return {
    start: function() {
      ...
      return hbase.getAllRows({
        ...
        for (const row of rows) {
          ...
          rowkeys.push(makeRowkey(row.rowkey, row.sequence))
        }
        ...
        log.info(('cached manifests: 0').green);
        return Promise.resolve();
      });
    },
    ...
    handleManifest: function(data) {
      ...
      return new Promise((resolve, reject) => {
        ...
        const key = makeRowkey (manifest.master_public_key, manifest.sequence);
